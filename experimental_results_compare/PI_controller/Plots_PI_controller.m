...
const_dist = 0.4;
%% loading data for constant velocity 0.07 m/s with PI controller, all sensors frame rate =5

ego_bot_vel_pt07_load = load('V_x_relsave_plot_all.mat');
ego_bot_vel_pt07 = ego_bot_vel_pt07_load.V_x_rel_save;
ego_bot_dist_pt07_load = load('x_relsave_plot_all.mat');
...
%% loading data for constant velocity 0.07 m/s with PI controller, LDS sensors frame rate =5
...
xlabel('Time')
ylabel('velocity (in m/s)')
title(' velocity of lead and ego vehicle by PI controller (m/s), LDS sensors')
legend('velocity of ego vehicle', 'velocity of lead vehicle')
hold off


%% loading data for constant velocity 0.07 m/s with PI controller, camera sensors frame rate =05
...
xlabel('Time')
ylabel('velocity (in m/s)')
title(' velocity of lead and ego vehicle by PI controller (m/s), camera sensors')
legend('velocity of ego vehicle', 'velocity of lead vehicle')
hold off

%% loading data for variable velocity with PI controller, all sensors frame rate =5
...
shift_arry_7_ele = 0.1*ones(1,39);
...
legend('relative distance of lead and ego vehicle', 'constant relative distance to maintain = 0.4m')
ax(2) = subplot(2,1,2);
plot(indx_pt07(1:135), ego_bot_vel_pt07(1:135), 'r', indx_pt07(1:135), lead_bot_vel_vari(1:135), 'b')
xlabel('Time')
ylabel('velocity (in m/s)')
