...
ego_bot_vel_vari_load=load('Sensor_Speed_Trace_StepVel.mat');
lead_bot_vel_vari = ego_bot_vel_vari_load.lead_bot_vel_vari;
%const_dist_plot = const_dist*ones(length(ego_bot_dist_vari),1);

const_vel_plot = 0.2*ones(length(lead_bot_vel_vari),1);
shift_arry_7_ele = lead_bot_vel_vari(1:30);
...
hold on
ax(1) = subplot(2,1,1);
plot(indx_pt07(1:160), ego_bot_dist_pt07(1:160), 'r', indx_pt07(1:160), const_dist_plot(1:160), 'b')
xlabel('Time')
ylabel('distance (in m)')
title(' distance(measured by sensor) between lead and ego vehicle (m), all sensors')
legend('relative distance of lead and ego vehicle', 'constant relative distance to maintain = 0.4m')
ax(2) = subplot(2,1,2);
plot(indx_pt07(1:160), ego_bot_vel_pt07(1:160), 'r', indx_pt07(1:160), lead_bot_vel_vari(1:160), 'b')
xlabel('Time')
ylabel('velocity (in m/s)')
